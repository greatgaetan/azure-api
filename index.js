 ...
 */
const WELCOME_MESSAGE = "Bonjour, bienvenue sur SandWish, le souhait des sandwiches! Qu'est-ce que je peux faire pour vous ? 🤖";
const SCHEDULE = "Nos sommes ouvert tous les jours sauf le dimanche, de 9h à 17h30! 🕐";
const HUNGRY_MSG = ['Personnellement, ça me donne très faim. 😋', 'Ca me donne un petit creux de parler de ça... 🍴', "Je sais pas vous, mais j'aurais bien pris celui-ci moi. 😏"]
...
APP.get("/price", async (req, res) => {
    ...
    req.rawHeaders[11].split(";").forEach(allSandwichInfo => {
        ...
        try {
            ...
            totalPrice += parseInt(sandwichInfo[0]) * (MENU.sandwiches[0][sandwichInfo[1].toLowerCase()].price);
        } catch (e) {
            return;
        }
    });
        ...
        await fetch(`http://api.openweathermap.org/geo/1.0/direct?q=${city}, belgium&limit=10&appid=${CONFIG.OPENWEATHERAPI_KEY}`)
            ...
            .then(async (data) => {
                ...
                var options = {
                    ...
                    params: { origins: restaurantLocation, destinations: `${data[0].lat},${data[0].lon}` },
                    headers: {
                ...
                await axios.request(options).then((response) => {
                    ...
                    finalRes = "Désolé, je n'ai pas réussi à calculer le temps de livraison. Pouvez-vous me redonner votre adresse svp?";
                });
            })
            .catch((error) => {
...
async function getWeatherInfo(lat, lon) {
    ...
    await fetch(url)
        ...
        .then(function (data) {
            ...
            var troubleMsg = (weatherId >= 500 && weatherId < 700) ? "nous aurons peut-être un peu de retard, veuillez nous excuser si c'est le cas." : "nous ne devrions pas avoir de retard.";
